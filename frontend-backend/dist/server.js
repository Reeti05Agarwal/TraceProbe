...
async function createKibanaSpace(spaceId, spaceName) {
...
async function createIndexPattern(spaceId, indexPatternId, title, timeField = "timestamp") {
    const headers = { "kbn-xsrf": "true", "Content-Type": "application/json" };
    const body = { attributes: { title, timeFieldName: timeField } };
    await axios.post(`${KIBANA_URL}/s/${encodeURIComponent(spaceId)}/api/saved_objects/index-pattern/${encodeURIComponent(indexPatternId)}`, body, { headers });
}
